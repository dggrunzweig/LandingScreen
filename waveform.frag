...
void main() {
  ...
  vec2 noise_st = gl_FragCoord.xy / 3.0;
  noise_st.x = floor(noise_st.x);
  noise_st.y = floor(noise_st.y);
  noise_st = noise_st.xy / u_resolution;
 
  vec3 color = generateWaveformGrid(st, u_rate, u_phase);
  vec3 noise;
  float noise_value = 0.02 * rand2d(noise_st);
