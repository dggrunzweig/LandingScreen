...
export function createAudioBuffer(n_channels: number, n_frames: number, sample_rate: number): AudioBuffer {
    // create a buffer
...
export function createBiquadFilter(ctx: AudioContext, type: BiquadFilterType, frequency: number, Q: number, gain_dB: number): BiquadFilterNode {
...
export function createOscillator(ctx: AudioContext, type: OscillatorType, frequency: number, detune: number): OscillatorNode {
...
export function createStereoPanner(ctx: AudioContext, pan: number): StereoPannerNode {
    const panner = ctx.createStereoPanner();
    panner.pan.value = pan;
    return panner;
}

export function createBufferSource(ctx: AudioContext, buffer: AudioBuffer, playback_rate: number, loop: boolean): AudioBufferSourceNode {
...
export function CreateNoiseOscillator(ctx: AudioContext, gain_dB = 0, duration = 2): AudioBufferSourceNode {
    // 2 second noise loop
...
export function createCompressor(ctx: AudioContext, thresh_db: number, knee_db: number, ratio: number, attack_s: number, release_s: number): DynamicsCompressorNode {
...
export function createDigitalDelay(ctx: AudioContext, delay_time: number, feedback_db: number): DigitalDelay {
...
export function createWaveShaper(ctx: AudioContext, table: Float32Array, oversample = <OverSampleType>"4x"): WaveShaperNode {
...
export async function createReverb(audio_ctx: AudioContext, t_60: number, lpf_fc = 14000, direct = false): Promise<ConvolverNode> {
    ...
    const lf_k_decay = 0.97 * t_60 / (6.9 / 4); // e^-6.9 = -60dB. Existing envelop function is e^-4
    // lf scale
    ...
    const noise_table = new Array<number>(noise_table_len).fill(0).map((_) => { return 2 * Math.random() - 1 });
    ...
    for (let c = 0; c < 2; ++c) {
        ...
        for (let i = 0; i < sample_len; ++i) {
            const t = i / fs;
            const lf_sample = lf_scale * noise_table[(i * (10 * c + 1)) % noise_table_len] * envelop(t, lf_k_onset, lf_k_decay);
...
export function CreateBufferFromFile(context: AudioContext, url: string): Promise<AudioBuffer | undefined> {
...
// turn off aec will turn off the aec, which is useful for using a loopback like blackhole
// this feature is only supported on firefox
// bug report in chrome: https://bugs.chromium.org/p/chromium/issues/detail?id=796964
export function CreateLiveInputNode(ctx: AudioContext, turn_off_aec = false): Promise<MediaStreamAudioSourceNode> {
    return new Promise(function (resolve, reject) {
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            navigator.mediaDevices
                ...
                .then((stream) => {
                    ...
                    track.applyConstraints(current_settings).then(() => {
...
export function RampToValue(param: AudioParam, start_time: number, value: number, duration: number) {
    param.setTargetAtTime(value, start_time, duration);
}

export function linearRampToValueAtTime(ctx: AudioContext, param: AudioParam, value: number, duration: number) {
...
export function TiltedHannWindow(N: number, tilt: number): Float32Array {
...
export function CropAudioBuffer(buffer: AudioBuffer, start_sample: number, end_sample: number): AudioBuffer {
    const n_channels = buffer.numberOfChannels;
...
export function NoteToPitch(note: string, octave: number, custom_root_hz?: number): number {
    octave = clamp(octave, 0, 8); // limit range to 0-8
    octave = Math.floor(octave); // no fractional octaves
    const C0 = custom_root_hz == undefined ? 16.35 : custom_root_hz; // Assuming A = 440
...
export function triangle(amp: number, period: number, t: number): number {
    return (4 * amp / period) * Math.abs(((t - period / 4) % period) - period / 2) - amp;
}

export function envelop(t: number, attack: number, release: number): number {
    if (t <= attack) {
        // e^0.7 = 2
...
// eighth note = 1/8
// etc
export function BPMToTime(BPM: number, division: number): number {
    const inv_div = 1 / division;
    return (60.0 / (BPM * inv_div / 4))
}

export function CheckPRNG(prng: Function | undefined): Function {
    if (prng == null) {
...
export function lerp(x: number, x_l: number, x_u: number, y_l: number, y_u: number): number {
    x = clamp(x, x_l, x_u);
...
export function createFilter(coeffs?: BiquadCoeffs): Filter {
    if (coeffs == undefined)
        return { coeffs: { b0: 0, b1: 0, b2: 0, a0: 0, a1: 0, a2: 0 }, x_z: [0, 0], y_z: [0, 0] };
    else
...
export function FilterSample(f: Filter, x: number): number {
    const y = (x * f.coeffs.b0 + f.x_z[0] * f.coeffs.b1 + f.x_z[1] * f.coeffs.b2 - f.y_z[0] * f.coeffs.a1 - f.y_z[1] * f.coeffs.a2) / f.coeffs.a0;
    f.y_z[1] = f.y_z[0];
    f.y_z[0] = y;
    f.x_z[1] = f.x_z[0];
    f.x_z[0] = x;
...
// function loadAudioWorkletModule(audio_ctx: AudioContext, moduleURL: string): Promise<void> {
...
// export function LoadModules(audio_ctx: AudioContext, urls: string[]): Promise<void> {
