...
class LFO {
  ...
  constructor(
      audio_context: AudioContext, rate_hz: number, min_val: number,
      max_val: number) {
  ...
  setRange(min_val: number, max_val: number): void {
    ...
    this.r.gain.setValueAtTime(range / 2, this.c.currentTime);
    this.os.offset.setValueAtTime(offset, this.c.currentTime);
  }
  ...
  connect(node_param: AudioParam): void {
    node_param.setValueAtTime(0, this.c.currentTime);
